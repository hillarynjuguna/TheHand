...
def load_persisted_artifact(job_id: str, artifact_id: str) -> Optional[dict]:
    path = artifact_dir_for(job_id) / f"{artifact_id}.json"
    if not path.exists():
        return None
    try:
...
def record_artifact_event(event_id: str, artifact_id: str, job_id: str, event_type: str, status: str, detail: dict | None, created_at: str) -> None:
    with store.transaction() as conn:
        conn.execute(
            "INSERT INTO artifact_events (event_id, artifact_id, job_id, event_type, status, detail_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?);",
            (event_id, artifact_id, job_id, event_type, status, json.dumps(detail or {}), created_at),
        )


def list_artifact_events(artifact_id: str, limit: int) -> list[dict]:
    with store.DB_LOCK, store.get_connection() as conn:
        rows = conn.execute(
            "SELECT event_id, artifact_id, job_id, event_type, status, detail_json, created_at FROM artifact_events WHERE artifact_id = ? ORDER BY created_at DESC LIMIT ?;",
            (artifact_id, limit),
        ).fetchall()
        return [dict(row) for row in rows]
